...
def delete_document(collection_name: str, doc_id: str) -> None:
    _col(collection_name).delete(ids=[doc_id])
...
def purge_error_documents(collection_name: Optional[str] = None) -> Dict[str, int]:
    ...
    for name in collections:
        ...
        bad = [
            doc_id for doc_id, text in zip(ids, docs) if _looks_like_error(text or "")
        ]
        if bad:
            col.delete(ids=bad)
...
def store_portfolio_analysis(
    portfolio_id: str, text: str, extra: Dict = None, doc_id: str | None = None
):
    doc_id = doc_id or f"portfolio_{portfolio_id}_{datetime.utcnow().timestamp()}"
...
def store_risk_assessment(
    ...
    doc_id = doc_id or f"risk_{portfolio_id}_{datetime.utcnow().timestamp()}"
...
def store_market_analysis(
    ...
    doc_id = doc_id or f"market_{symbol}_{datetime.utcnow().timestamp()}"
...
def store_compliance_report(
    ...
    doc_id = doc_id or f"compliance_{portfolio_id}_{datetime.utcnow().timestamp()}"
...
def search_portfolio_analysis(
    query: str, portfolio_id: str = None, analysis_type: str = None, n: int = 5
):
    filters = []
    if portfolio_id is not None:
        filters.append({"portfolio_id": str(portfolio_id)})
    filters.append({"record_type": "analysis"})
    if analysis_type:
