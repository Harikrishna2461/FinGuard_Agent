...
def run_check(cmd: list[str], cwd: Path, env: Optional[dict[str, str]] = None, timeout_s: int = 300) -> tuple[bool, str]:
...
def build_doc() -> Document:
    ...
    _p(doc, "This report is generated by reviewing and executing the application code (backend + frontend), plus running the included test scripts. It maps what is implemented vs what is still missing against the AAS Practice Module Briefing expectations (v4.2).")
    ...
    _add_kv_table(
        ...
        [
            ...
            ("Briefing extract", "docs/aas_practice_module_briefing_v4.2_full_time.txt"),
    ...
    _p(doc, "Backend modules reviewed: app factory/bootstrapping, API routes, auth, audit log, case management, SAR export, agent orchestrator + tools, ML engine, vector store.")
    _p(doc, "Frontend modules reviewed: React pages under frontend/src/pages and the Flask template UI under backend/app/templates/index.html.")

    _h1(doc, "2. Backend capabilities observed in code")
    _h2(doc, "2.1 Portfolio management + analysis")
    _p(doc, "Implemented: CRUD-style portfolio creation, asset addition/listing, transaction recording, alerts, and AI analysis endpoints.")
    _p(doc, "Evidence: backend/app/routes.py")

    _h2(doc, "2.2 Hybrid ML risk scoring")
    _p(doc, "Implemented: hybrid scoring (rules + ML models) with rule flags, ML details, hard-block logic, and optional LLM review indicator.")
    ...
    _p(doc, "Implemented: 9 specialist agents. Orchestrator runs 3 sequential crews to manage rate limits. Agents can use tool functions and RAG context from ChromaDB knowledge base.")
    _p(doc, "Evidence: backend/agents/crew_orchestrator.py, backend/agents/*, backend/vector_store.py")

    _h2(doc, "2.4 Analyst workflow: cases, audit trail, SAR export")
    _p(doc, "Implemented: tenant-scoped case queue with state machine transitions; append-only tamper-evident audit log; SAR worksheet export in JSON and PDF.")
    _p(doc, "Evidence: backend/app/cases.py, backend/app/audit.py, backend/app/sar.py, backend/models/models.py")

    _h2(doc, "2.5 Authentication / authorization")
    _p(doc, "Implemented: JWT auth with roles (analyst/supervisor/admin). Auth enforcement is configurable (AUTH_ENFORCED). Tenant isolation is applied in cases/audit/SAR.")
    ...
    if pages_dir.exists():
        ...
        _p(doc, "This table is derived by scanning each React page source file for API calls (fetch/axios) to /api endpoints.")
        _add_table(
            doc,
    ...
    _p(doc, "Implemented: a working demo UI served at / (backend/app/templates/index.html) that calls many backend endpoints (portfolios, transactions risk scoring, AI analysis, search, sentiment, cases).")
    _p(doc, "Evidence: backend/app/__init__.py, backend/app/templates/index.html")

    _h1(doc, "4. Runtime checks (tests executed)")
    _p(doc, "These checks were executed using the current Python environment. The integration test is run with a placeholder GROQ_API_KEY; it does not make external LLM calls.")

    checks = []

    ok1, out1 = run_check([sys.executable, "-m", "ml.test_engine"], cwd=BACKEND_DIR)
    checks.append(("ML engine test (ml.test_engine)", "PASS" if ok1 else "FAIL", out1[:1500]))

    ok2, out2 = run_check([sys.executable, "test_knowledge_base.py"], cwd=BACKEND_DIR)
    checks.append(("Knowledge base test (test_knowledge_base.py)", "PASS" if ok2 else "FAIL", out2[:1500]))
    ...
    checks.append(("Integration test (test_integration.py)", "PASS" if ok3 else "FAIL", out3[:1500]))

    _add_table(doc, headers=["Check", "Result", "Notes (truncated)"], rows=[list(c) for c in checks])

    _h1(doc, "5. Mapping to Practice Module Briefing — Implemented vs Missing")
    _p(doc, "This section maps the briefing’s ‘Success criteria’ artifact list (system architecture, agent design, XAI/responsible AI, AI security risk register, MLSecOps/LLMSecOps pipeline, tests, UI prototype) to what is present in this repository as working code and runnable checks.")
    ...
    _add_table(doc, headers=["Briefing item", "Status", "Evidence / notes"], rows=mapping_rows)
