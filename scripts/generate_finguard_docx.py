...
def build_document() -> Document:
    ...
    _add_kv_table(
        ...
        [
            ...
            (
                "Scope",
                "Backend Flask API, CrewAI agents, ML risk engine, ChromaDB, UI pages",
            ),
    ...
    doc.add_paragraph(
        "3. Auto create alerts and open investigation cases for higher risk activity"
    )
    doc.add_paragraph(
        "4. Run AI analysis across nine specialist agents for portfolio review"
    )
    doc.add_paragraph(
        "5. Provide search over prior AI outputs stored in the vector database"
    )
    ...
    _add_kv_table(
        ...
        [
            ...
            ("Frontend", "React analyst UI plus a Flask template demo UI"),
        ],
    )
    ...
    _add_kv_table(
        ...
        [
            ...
            (
                "AUTH_ENFORCED",
                "Optional. If true then a JWT bearer token is required for cases and SAR endpoints",
            ),
    ...
    doc.add_paragraph(
        "2. This will install Python dependencies and build the React frontend if needed, then start Flask. "
    ...
    agent_rows = [
        (
            "Alert Intake Agent",
            "Processes and categorizes alerts and can enrich transaction alerts with ML risk scores",
        ),
        ("Customer Context Agent", "Builds and summarizes customer profiles and needs"),
        (
            "Risk Assessment Agent",
            "Scores portfolio risk and transaction risk with a hybrid rules and ML pipeline and LLM explanation",
        ),
        (
            "Risk Detection Agent",
            "Detects fraud patterns and suspicious behavior and assesses market risk exposure",
        ),
        (
            "Compliance Agent",
            "Reviews for regulatory and policy compliance and tax reporting issues",
        ),
        (
            "Portfolio Analysis Agent",
            "Evaluates allocation and performance and recommends rebalancing actions",
        ),
        (
            "Market Intelligence Agent",
            "Produces market sentiment and investment recommendations for symbols",
        ),
        (
            "Explanation Agent",
            "Explains risk scores findings and recommendations in clear language",
        ),
        (
            "Escalation Case Summary Agent",
            "Decides escalation need and prepares case summaries and escalation packages",
        ),
    ]
    ...
    doc.add_paragraph(
        "Crew 1 order: Risk Assessment, then Risk Detection, then Compliance"
    )
    doc.add_paragraph(
        "Crew 2 order: Portfolio Analysis, then Market Intelligence, then Customer Context"
    )
    ...
    doc.add_paragraph(
        "The repository includes two user interfaces. The primary React frontend is the main analyst experience, while the Flask template UI remains a working demo interface. "
    ...
    _add_kv_table(
        ...
        [
            ...
            (
                "Sentiment",
                "Calls the backend to fetch symbols and request sentiment analysis",
            ),
    ...
    _add_kv_table(
        ...
        [
            ...
            (
                "Portfolios",
                "Create portfolio, view portfolio, add assets, view transactions",
            ),
            (
                "Transaction Risk Analysis",
                "Record a transaction and score risk and open a case if high risk",
            ),
            (
                "AI Analysis",
                "Run full portfolio analysis and request an investment recommendation",
            ),
            ("Alerts", "View alerts"),
            ...
            (
                "Cases",
                "Analyst case queue and case detail with state transitions and SAR export",
            ),
        ],
    )
    ...
    doc.add_paragraph(
        "Purpose: provide AI based market sentiment for one or more symbols"
    )
    ...
    doc.add_paragraph(
        "2. If the risk score is high then an alert is created and a case may be auto opened"
    )
    doc.add_paragraph(
        "3. Optional: the page then calls Explanation agent to generate human readable insights"
    )
    ...
    doc.add_paragraph(
        "Crew 2: Portfolio Analysis, then Market Intelligence, then Customer Context"
    )
    ...
    doc.add_paragraph(
        "Purpose: request an investment recommendation for a selected symbol"
    )
    doc.add_paragraph("Backend endpoint:")
    ...
    doc.add_paragraph(
        "Purpose: semantic search over prior AI outputs stored in the vector database"
    )
    doc.add_paragraph("Backend endpoints:")
    ...
    doc.add_paragraph(
        "4. recommendation action uses a single Risk Assessment agent crew"
    )
    ...
    doc.add_paragraph(
        "5. If the score is high, review the auto created case in the case queue"
    )
    ...
    api_rows = [
        ...
        (
            "Portfolio",
            "POST /api/portfolio, GET /api/portfolios, GET /api/portfolio/<id>",
        ),
        ("Assets", "POST /api/portfolio/<id>/asset, GET /api/portfolio/<id>/assets"),
        (
            "Transactions",
            "POST /api/portfolio/<id>/transaction, GET /api/portfolio/<id>/transactions",
        ),
